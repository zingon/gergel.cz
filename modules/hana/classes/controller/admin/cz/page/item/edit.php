...
class Controller_Admin_Page_Item_Edit extends Controller_Hana_Edit
{
    ...
    protected function _column_definitions()
    {
        ...
        {
            $this->auto_edit_table->row("parent_id")->type("selectbox")->label("Nadřazená stránka")->item_settings(array("max_tree_level"=>$this->max_category_level))->data_src(array("column_name"=>"nazev","orm_tree"=>true,"null_row"=>"---","language"=>true,"condition"=>array("page_category_id","=",$this->page_category)))->set();
        ...
        $this->auto_edit_table->row("show_child_pages_index")->type("checkbox")->label("Vložit náhledy podstránek")->set();
        $this->auto_edit_table->row("show_contactform")->type("checkbox")->label("Vložit kontaktní formulář")->set();
        $this->auto_edit_table->row("show_visitform")->type("checkbox")->label("Vložit formulář - návštěva zástupce")->set();
        ...
        $this->auto_edit_table->row("akce_text")->type("edit")->label("Úvodní text - akce")->set();
