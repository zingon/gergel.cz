...
abstract class Controller_Hana_Photoedit extends Controller_Hana_Edit {
    ...
    protected $subject;
    protected $subject_suffix="_photo";
    protected $subject_col_id_name;
    protected $subject_photos_name;

    protected $table_template = "admin/edit/gallery";
    ...
    public function before() {
        ...
        $this->table->back_to_item=url::base().Kohana::$index_file."admin/".i18n::lang()."/".$this->module_key."/".$this->submodule_key."/edit/1/".$this->item_id;
    ...
    protected function _column_definitions()
    {
        ...
        if($this->admlang==1)
            $this->auto_edit_table->row("link_new")->type("photoeditlinks")->item_settings(array("href"=>$this->base_admin_path."?".$main_gallery_key))->variant("one_col")->set();


        $this->auto_edit_table->row($main_gallery_key)->type("photoedit")->data_src(array("image_dir"=>$this->image_dir,"href"=>$this->base_admin_path,"photos_orm_name"=>$this->subject."_photo"))->variant("one_col")->set();
...
//        $this->auto_edit_table->row("main_image")->type("image")->item_settings(array("dir"=>$this->image_dir,"suffix"=>"at","ext"=>"jpg","delete_link"=>true))->label("Náhled obrázku")->set();
    ...
    protected function _form_action_photoedit_main_gallery_add($data)
    {
        ...
        if(empty($errors))
        {
            ...
            if($data["photo_id"])
            {
                $subject_photos=orm::factory($this->subject_photos_name)->language($data["language_id"])->where($this->subject."_photos.id","=",$data["photo_id"])->find();
            }
            else
            {
