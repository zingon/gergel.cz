...
class Service_Hana_Product_Category implements Interface_Searchable 
{
    ...
    public static function get_product_category_by_route_id($route_id=false, $parameters=array())
    {
        ...
        //die($products);
        $products->where("routes.zobrazit","=",1);
        ...
        foreach($products as $product)
        {
           ...
           $current_product["cena_bez_dph"] = orm::factory("product")->where("products.id","=",$product["id"])->find()->prices()->get_total_price_without_tax();
    ...
    public static function get_carousel_product_categories($nazev_seo, $language_id)
    {
        ...
        foreach($product_categories as $category)
        {
           ...
           $products=DB::select("product_data.id","product_data.nazev","routes.nazev_seo")->from("product_data")
           ...
           $product_category_data[$category->id]["products"]=$products;
...
//        
     ...
     * Metoda pro nalezeni vsech produktů pro feed heureka a zbozi
     *      */
	...
	$products = DB::select("products.*")
			->select("product_data.nazev", "product_data.uvodni_popis", "product_data.description", "product_data.popis", "product_data.akce_text")
			//->select(array("manufacturer_data.nazev","vyrobce"))
			->select(array("routes2.nazev_seo", "product_nazev_seo"))->distinct(true);

	$special_mode = false;

	$products = self::_category_query_base($products, $special_mode);
	$products = self::_category_query_base_condition($products, false, $search_string);
	$products = self::_category_query_filters_condition($products, false);
	//$products->join("product_data")->on("products.id","=","product_data.product_id")
	$products->join("manufacturer_data", "left")->on("manufacturers.id", "=", "manufacturer_data.manufacturer_id");
	$products->join(array("routes", "routes2"))->on("product_data.route_id", "=", "routes2.id");
	...
	if (isset($parameters["limit"]))
	    $products->limit($parameters["limit"]);
	if (isset($parameters["offset"]))
	    $products->offset($parameters["offset"]);
	//die($products);

	$products = $products->execute();

	foreach ($products as $product) {
	    $products_cat = DB::select("product_category_data.title", "product_categories.parent_id")
		    ->from('product_categories')
		    ->join("product_categories_products")->on("product_categories.id", "=", "product_categories_products.product_category_id")
		    ->join("product_category_data")->on("product_categories.id", "=", "product_category_data.product_category_id")
		    ->where('product_categories_products.product_id', '=', $product["id"])
		    ->group_by('product_categories.parent_id')
		    ->order_by('product_categories.parent_id');

	    $products_cat = $products_cat->execute();
	    $current_product = $product;
	    $current_product['categories'] = $products_cat->as_array();
	    $current_product["nazev_seo"] = $product["product_nazev_seo"];
	    $current_product["cena_s_dph"] = orm::factory("product")->where("products.id", "=", $product["id"])->find()->prices()->get_total_price_with_tax();

	    // cesta k obrazku
	    $filename = self::$photos_resources_dir . "product/item/images-" . $product["id"] . "/" . $product["photo_src"] . "-" . $photo_detail_suffix . ".jpg";
	    if ($product["photo_src"] && file_exists(str_replace('\\', '/', DOCROOT) . $filename)) {
		$current_product["photo"] = url::base() . $filename;
	    }
	    // zaverecne vlozeni produktu do pole dat
	    $product_data[$product["id"]] = $current_product;
	}
