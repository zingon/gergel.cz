...
class Filemanager {
    ...
    protected $logfile = '/application/logs/filemanager/log.log';
    ...
    public function __construct($extraConfig = '') {
        ...
        $this->root = preg_replace("!{$_SERVER['SCRIPT_NAME']}$!", '', $_SERVER['SCRIPT_FILENAME']);
        // getting default config file
        $content = file_get_contents($this->root."/media/admin/js/filemanager/scripts/filemanager.config.js.default");
        $config_default = json_decode($content, true);

        // getting user config file
        $content = file_get_contents($this->root."/media/admin/js/filemanager/scripts/filemanager.config.js");
        ...
        if ($this->config['options']['fileRoot'] !== false ) {
            ...
            if ($this->config["options"]["dgstudio"] && count($host) > 0 )
            {
                $last_index = count($host)-1;
                $this->doc_root .= '/'.$host[$last_index-1].'.'.$host[$last_index];
                $j = true;
                for($i = $last_index - 2; $i >= 0; $i--)
                {
                    if ($j)
                        $this->doc_root .= '/domains';
                    $j = false;
                    $this->doc_root .= '/'.$host[$i];
                }
            }
        ...
        if($this->is_image($imagePath) && $this->config['images']['resize']['enabled']) {
            if ($size = @getimagesize($imagePath)){
                if ($size[0] > $this->config['images']['resize']['maxWidth'] || $size[1] > $this->config['images']['resize']['maxHeight']) {
                    require_once($this->getFullPath('/media/admin/js/filemanager/connectors/php/inc/vendor/wideimage/lib/WideImage.php'));
    ...
    public function preview($thumbnail) {
        ...
        if(isset($this->get['path']) && file_exists($current_path)) {
            ...
            $returned_path =  substr(preg_replace("@[/]+@i", "/", $returned_path), strpos($returned_path, "/media"));/*
    ...
    private function get_file_info($path='', $thumbnail = false) {
        ...
        $b_path = $this->get["path"];
        if (!preg_match("/". $tmp[(sizeof($tmp)-1)]."/i", $this->get["path"]))
            $this->get["path"] .= DIRECTORY_SEPARATOR.$this->item["filename"];
        ...
        if(is_dir($this->getFullPath($current_path))) {
            ...
            if($this->item['filetype'] == 'svg') {
                ...
                $this->item['preview'] = $this->preview($thumbnail);
                //$this->item['preview'] = '/admin/file_manager?mode=preview&path='. rawurlencode($current_path).'&'. time();
                //if($thumbnail) $this->item['preview'] .= '&thumbnail=true';
                $this->get["path"] = $b_path;
            }
    ...
    private function loadLanguageFile() {
        ...
        if(file_exists($this->root. 'scripts/languages/'.$lang.'.js')) {
            $stream =file_get_contents($this->root. '/media/admin/js/filemanager/scripts/languages/'.$lang.'.js');
    ...
    private function availableLanguages() {
        ...
        if ($handle = opendir($this->root.'/media/admin/js/filemanager//scripts/languages/')) {
            while (false !== ($file = readdir($handle))) {
                if ($file != "." && $file != "..") {
    ...
    private function __log($msg) {
        ...
        if($this->logger == true) {
            $fp = fopen($this->root.$this->logfile, "a");
