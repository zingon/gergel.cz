...
class Kohana_ORM {
	...
	public function reload_columns($force = FALSE)
	{
		...
		{
                    // doplnek pro zamezeni znovunacitani full columns _data tabulek (pri problemech smazat, nebo opravit jinak)
                    if($this->_object_name=="language_data"){
                        $newobj=explode("_",$this->_table_name);
                        $this->_object_name=$newobj[0];
                    } // konec
			...
			else
			{
				// Grab column information from database
                                //echo("[".$this->_table_name."][".$this->_object_name."]<br />");
	 ...
	 * @return  ORM
	 */
	public function add($alias, $model, $data = NULL)
	{
	...
	/**
	 * Count the number of records in the table. PREIMPLEMENTOVANO !!!!!!!!
	 *
	 * @return  integer
	 */
	public function count_all($condition="*")
	{
		...
		$records = (int) $this->_db_builder->from($this->_table_name)
			->select(array(DB::expr("COUNT(".$condition.")"), 'records_found'))
			->execute($this->_db)
			->get('records_found');
