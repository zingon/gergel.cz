...
class Service_Catalog extends Service_Hana_Module_Base
{
    ...
    public static function get_catalog_items_list($category_id, $language_id,$limit=100,$offset=0,$homepage=false)
    {
        $products=orm::factory("product")
                ...
                //->where("catalog_category_id","=",db::expr($category))
                ->where("zobrazit","=",1)
        ...
        foreach ($products as $product)
        {
            ...
            $dirname=self::$photos_resources_dir.self::$navigation_module."/item/images-".$product->id."/";

            if($product->photo_src && file_exists(str_replace('\\', '/',DOCROOT).$dirname.$product->photo_src."-t3.jpg"))
            {
                $result_data[$product->id]["photo_detail"]=url::base().$dirname.$product->photo_src."-t3.jpg";
            }
            else
            {
                $result_data[$product->id]["photo_detail"]="";
            }
